/* assets/js/img-enhance.js */
/* Robust skeleton shimmer that RESERVES SIZE and transitions cleanly.
   Works whether images use data-src/data-lqip or plain <img src>. */
(function () {
  "use strict";

  const CDN_PREFIX = (typeof window !== "undefined" && window.__IMG_CDN__) || "";
  const SELECTOR_SCOPE = ".post-content";

  function withCDN(url) {
    if (!url || !CDN_PREFIX) return url;
    return url.startsWith("/") ? String(CDN_PREFIX).replace(/\/+$/, "") + url : url;
  }

  function ensureGLightbox(onReady) {
    if (window.GLightbox) return onReady && onReady();
    const css = document.createElement("link");
    css.rel = "stylesheet";
    css.href = "https://cdn.jsdelivr.net/npm/glightbox/dist/css/glightbox.min.css";
    document.head.appendChild(css);
    const s = document.createElement("script");
    s.src = "https://cdn.jsdelivr.net/npm/glightbox/dist/js/glightbox.min.js";
    s.onload = function () { onReady && onReady(); };
    document.head.appendChild(s);
  }

  /* ---------- Captions → Lightbox ---------- */
  function getCaptionFromDOM(img) {
    if (img.dataset.caption) return img.dataset.caption.trim();
    const fig = img.closest("figure");
    const fc = fig && fig.querySelector("figcaption");
    if (fc && fc.textContent) return fc.textContent.trim();
    const sib = img.nextElementSibling;
    if (sib && sib.classList.contains("image-caption") && sib.textContent) return sib.textContent.trim();
    if (img.alt && img.alt.trim()) return img.alt.trim();
    return "";
  }
  function applyCaption(img, anchor) {
    const cap = getCaptionFromDOM(img);
    if (!cap) return;
    if (!anchor) anchor = img.closest("a.glightbox");
    if (!anchor) return;
    anchor.setAttribute("data-title", cap);
    anchor.setAttribute("title", cap);
    anchor.setAttribute("aria-label", cap);
  }

  /* ---------- Lightbox wrapper (idempotent) ---------- */
  function upgradeOrWrapForLightbox(img) {
    let a = img.closest("a.glightbox");
    if (a) return a;

    const parentAnchor = img.closest("a");
    const href = img.dataset.full || img.dataset.src || img.currentSrc || img.src;

    if (parentAnchor) {
      parentAnchor.classList.add("glightbox");
      if (href) parentAnchor.href = href;
      parentAnchor.dataset.gallery = parentAnchor.dataset.gallery || "post";
      return parentAnchor;
    }

    const p = img.parentElement;
    if (!p) return null;

    a = document.createElement("a");
    a.className = "glightbox";
    if (href) a.href = href;
    a.dataset.gallery = "post";
    p.insertBefore(a, img);
    a.appendChild(img);
    return a;
  }

  /* ---------- Skeleton container around IMG (and its anchor) ---------- */
  function ensureSkeletonContainer(img) {
    // If the image (or its anchor) is already inside .img-skel, reuse it.
    let container = img.closest(".img-skel");
    if (container) return container;

    // Prefer to wrap the anchor + img together, so shimmer sits under the clickable area.
    const anchor = img.closest("a") || null;
    const target = anchor || img;
    const parent = target.parentElement;
    if (!parent) return null;

    container = document.createElement("span");
    container.className = "img-skel";
    // Insert container before target, then move target inside.
    parent.insertBefore(container, target);
    container.appendChild(target);

    // Add the shimmer layer
    const skel = document.createElement("span");
    skel.className = "skeleton-box";
    container.appendChild(skel);

    return container;
  }

  function computeWH(img) {
    // Use explicit attributes if provided
    let w = parseInt(img.getAttribute("width")  || img.dataset.w || "", 10);
    let h = parseInt(img.getAttribute("height") || img.dataset.h || "", 10);

    // If missing, try LQIP's natural size once it's assigned
    if ((!w || !h) && img.naturalWidth && img.naturalHeight) {
      w = img.naturalWidth; h = img.naturalHeight;
    }

    // Last fallback ratio to avoid layout collapse
    if (!w || !h) { w = 16; h = 9; }

    return { w, h };
  }

  function startSkeleton(img, container) {
    const { w, h } = computeWH(img);
    container.style.setProperty("--w", String(w));
    container.style.setProperty("--h", String(h));

    // If author gave fixed pixel width, cap the container so it doesn't grow larger.
    const declaredW = parseInt(img.getAttribute("width") || "", 10);
    if (declaredW) {
      container.style.setProperty("--maxw", declaredW + "px");
    }

    // Ensure the anchor/IMG is absolutely positioned to fill the box
    const anchor = img.closest("a");
    (anchor || img).style.position = "absolute";
    (anchor || img).style.inset = "0";
    (anchor || img).style.width = "100%";
    (anchor || img).style.height = "100%";

    // Hide the real image initially; it will fade in on 'load'
    img.style.opacity = "0";
  }

  function finishSkeleton(img, container) {
    // Reveal image
    img.style.opacity = "";
    container.classList.add("ready");

    // Allow content to keep filling the box; we keep container for layout.
    // (If you want to remove the container entirely, you'd need to re-parent nodes here.)
  }

  /* ---------- LQIP + real image swap (optional) ---------- */
  const EMPTY_DATA_URI =
    "data:image/png;base64,";

  function applyProgressiveLoad(img) {
    const realSrc = img.getAttribute("data-src") || img.dataset.src;
    const lqip   = img.getAttribute("data-lqip") || img.dataset.lqip;
    if (!realSrc) return false; // not using progressive path

    // 1) Ensure container + reserve footprint BEFORE setting sources
    const container = ensureSkeletonContainer(img);
    if (!container) return false;
    startSkeleton(img, container);

    // 2) Set initial visual (LQIP or tiny pixel)
    if (!img.getAttribute("src")) {
      img.setAttribute("src", lqip ? withCDN(lqip) : EMPTY_DATA_URI);
    } else if (lqip) {
      img.setAttribute("src", withCDN(lqip));
    }

    // 3) Preload the full-res image, then swap and finalize
    const actual = withCDN(realSrc);
    const loader = new Image();
    loader.decoding = "async";
    loader.onload = function () {
      // Swap to actual and wait for the IMG element's own load to fire
      img.addEventListener("load", function onRealLoad() {
        finishSkeleton(img, container);
        img.removeEventListener("load", onRealLoad);
      });
      img.src = actual;
    };
    loader.onerror = function () {
      // Even on error, attempt to swap; at least remove the shimmer
      img.src = actual;
      finishSkeleton(img, container);
    };
    loader.src = actual;

    if (loader.complete && img.src !== actual) {
      img.src = actual;
    }
    return true;
  }

  function enhanceImages() {
    const scope = document.querySelector(SELECTOR_SCOPE) || document.body;
    const imgs = scope.querySelectorAll("img:not([data-noenhance])");

    imgs.forEach(function (img) {
      // Normalize site-rooted paths through CDN
      const rawSrc = img.getAttribute("src");
      if (rawSrc && rawSrc.startsWith("/")) img.setAttribute("src", withCDN(rawSrc));

      // Always set good decoding hints
      img.loading = img.loading || "lazy";
      img.decoding = img.decoding || "async";

      // Progressive loader path (data-src / data-lqip) → full skeleton shimmer
      const usedProgressive = applyProgressiveLoad(img);

      // If not using progressive path, still give a *simple* skeleton if width/height exist
      if (!usedProgressive) {
        // Only if the image hasn't finished yet
        if (!img.complete) {
          const container = ensureSkeletonContainer(img);
          if (container) {
            startSkeleton(img, container);
            img.addEventListener("load", function onLoad() {
              finishSkeleton(img, container);
              img.removeEventListener("load", onLoad);
            });
          }
        }
      }

      // Lightbox & caption
      if (!img.dataset.nolightbox) {
        const a = (function () {
          // If we created a container, the anchor is inside it—upgrade that anchor.
          const existingA = img.closest("a");
          if (existingA && !existingA.classList.contains("glightbox")) {
            existingA.classList.add("glightbox");
            existingA.dataset.gallery = existingA.dataset.gallery || "post";
            return existingA;
          }
          return upgradeOrWrapForLightbox(img);
        })();
        if (a && img.dataset.full) a.href = withCDN(img.dataset.full);
        applyCaption(img, a);
      }
    });

    // Init GLightbox safely
    ensureGLightbox(function () {
      if (!window._glightboxInstance) {
        window._glightboxInstance = GLightbox({ selector: ".glightbox" });
      } else if (window._glightboxInstance.reload) {
        window._glightboxInstance.reload();
      }
    });
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", enhanceImages, { once: true });
  } else {
    enhanceImages();
  }

  // Expose for soft navigations if you add them later
  window.__enhanceImages = enhanceImages;
})();
